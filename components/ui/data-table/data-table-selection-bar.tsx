...
function DataTableSelectionBar({
  ...
  return (
    <div
      className={cn(
        "fixed bottom-6 left-0 right-0 z-50 flex justify-center px-4 transition-[padding] duration-200 pointer-events-none",
        open && "md:pl-[var(--sidebar-width)] md:pr-4",
      )}
    >
      <div
        className={cn(
          "pointer-events-auto max-w-full transition-all duration-200",
          visible ? "translate-y-0 opacity-100" : "translate-y-3 opacity-0 pointer-events-none",
        )}
      >
        <div className="flex items-center gap-1 rounded-full border bg-popover px-2 py-2 shadow-xl shadow-black/10 ring-1 ring-border/50 overflow-x-auto">
